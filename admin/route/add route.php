...
<?php 
    if(isset($_POST['saveprofile']))
    {
        ...
        if($bus_route_start === $bus_route_des)
        {
            ?>
                <script>
                     Swal.fire({
                      title: 'Error!',
                      html: 'The Route for Starting Point and Destination you have selected is the <b>Same</b>. <br>Please enter a different Starting Point and Destination.',
                      icon: 'error',
        ...
        {
            $check_starting_point = $row_add_check['starting_point'];
            $check_destination = $row_add_check['destination'];
            $check_driver_id = $row_add_check['driver_id'];
            

            if($bus_route_start === $check_starting_point && $bus_route_des === $check_destination)
            {
                ?>
                <script>
                     Swal.fire({
                      title: 'Error!',
                      html: 'The Route for Starting Point and Destination have existed. <br>Please enter a different Starting Point and Destination.',
                      icon: 'error',
                      showConfirmButton: true,
                      timer: 8500
                  });
            ...
            else if($bus_driver_id === $check_driver_id)
            {
                ?>
                <script>
                     Swal.fire({
                      title: 'Error!',
                      html: 'The Driver have been selected. <br>Please select a different Driver.',
                      icon: 'error',
                      showConfirmButton: true,
                      timer: 8500
                  });
        ...
        <script>
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        url: '/FYP/admin/route/add route process.php',
                        type: 'POST',
                        data: {
