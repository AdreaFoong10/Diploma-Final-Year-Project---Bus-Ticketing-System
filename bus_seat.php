...
<body>
        ...
        for($i=0;$i<$_SESSION["qtybuses"];$i++)
        {
            ...
            if(isset($_POST[$button_name1]))
            {
                if(isset($_POST["bsid"]))
                {
                    ...
                    $_SESSION['alight'] = $alight;
                    $_SESSION['departure_bushid'] = $bus_schid;
                    $_SESSION['departure_fare'] = $fare;
                    $_SESSION['departure_bus_schedule_date'] = $bus_schedule_date;
                    $_SESSION['summary_departure_time'] = $summary_departure_time;
                    $_SESSION['gateD'] = $row_sql['gate'];


                }

                $sql_bus = "SELECT bus_id FROM bus WHERE bus_schedule_id = $_SESSION[departure_bushid]";
                $get_busID = mysqli_query($conn,$sql_bus);
                $row_bus = mysqli_fetch_assoc($get_busID);

                
                $bus_id = $row_bus['bus_id'];
                $_SESSION['busID'] = $bus_id;
    ...
    <form method="post" action="bus_seat.php">
    <div class="seat-container">
        ...
        <div class="columnThree">
            ...
            <div style="display:flex;">
                <div class="info1"><img src="img/dot.png" style="width:25px;"><span><?php echo $_SESSION['board'] ?></span></div><br/>
            </div>

            <div style="display:flex; margin-top:5px;">
                <div class="info2"><img src="img/dot.png" style="width:25px;"><span><?php echo $_SESSION['alight'] ?></span></div>
            </div>
...
<?php
    ...
    {
        $sql_checkSeat = "SELECT bus_seat_no FROM bus_seat WHERE bus_schedule_id = $_SESSION[departure_bushid] AND bus_id = $_SESSION[busID] AND bus_seat_status = 1";
    ...
    if(isset($_POST['btnpro']))
    {
        ...
        foreach ($selectedSeats as $seat) {
            $_SESSION['bus_no'][$i] = $seat; // Store each seat value in the session array
            $i++;
        }
        ...
        if(isset($_SESSION['return_date']))
        {
            ?>
                <script>
                    ...
                    html: 'You will be direct to choose round trip ticket!',
                    timer: 2000,
                    ...
                    if (result.dismiss === Swal.DismissReason.timer) {
                        window.location.href ="return_product_list.php";
