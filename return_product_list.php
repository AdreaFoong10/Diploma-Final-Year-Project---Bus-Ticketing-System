...
<?php
        ...
        $departure = $_SESSION['destination'];
        $destination = $_SESSION['origin'];
        $departure_date = $_SESSION['return_date'];
        $return_date = $_SESSION['departure_date'];
        $error = 0;
        

        // if($departure == "emptyo" || $destination =="emptyd" || empty($departure_date))
        // {
        //     $error = 1;
        ...
        { 
            ...
            // $_SESSION['origin_return'] = $departure;
            // $_SESSION['destination_return'] = $destination;
        }
        ...
        <div style="margin:0px;">
            ...
            <div style="display:grid; grid-template-columns:auto auto;border-bottom: 1px solid black;">
                <div style="padding:5px; " id="departure_list"><?php echo $destination ?> > <?php echo $departure ?> > <?php echo $return_date ?></div>
                <?php
                ...
                {
                    ?>
                        <div style="padding:5px; background-color:#CECCCC; " id="return_list"><?php echo $departure ?> > <?php echo $destination ?> > <?php echo $departure_date ?></div>
                    <?php
                }
                ?>
            </div>
                ...
                <table>
                    <thead>
                        ...
                        <?php
                            ...
                            $row_schedule=mysqli_fetch_assoc($run_schedule);
                            
                            if(isset($row_schedule['bus_schedule_date']))
                            {
                                if($row_schedule['bus_schedule_date'] > $currentday)
                                {
                                    $get_schedule = "SELECT DISTINCT bus_operators.bus_operator_pic, bus_operators.bus_operator_name,bus_schedule.bus_schedule_id,bus_schedule.departure_time, bus_schedule.boarding,
                                                bus_schedule.duration, bus_schedule.arrival_time, bus_schedule.fare, bus_schedule.bus_schedule_date FROM bus_schedule 
                                                INNER JOIN bus_operators ON bus_schedule.bus_operator_id = bus_operators.bus_operator_id
                                ...
                                else
                                {
                                    $get_schedule = "SELECT DISTINCT bus_operators.bus_operator_pic, bus_operators.bus_operator_name,bus_schedule.bus_schedule_id,bus_schedule.departure_time, bus_schedule.boarding,
                                                ...
                                                WHERE bus_schedule.bus_schedule_status = 1 AND bus_schedule.route_id = $route_id
                                                AND bus_schedule.bus_schedule_date LIKE '%$departure_date%'
                                                AND bus_schedule.departure_time > $currenttime
                                                ORDER BY bus_schedule.fare";
                                    $x=0;
                                    $run_schedule = mysqli_query($conn,$get_schedule);
                                }
                            ...
                            <tr>
                                <form name="frmbsid<?php echo $x ?>" action="return_bus_seat.php" method="post">
...
<script>
    ...
    if(return_date == 1)
    {
        document.getElementById("return_list").innerHTML = "<a onclick='window.history.back()'> ADD A RETURN DATE</a>";
    }
</script>
